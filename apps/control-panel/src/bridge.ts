  ...
  if (command === "fetch_diagnostics") {
    ...
    return {
      ...
      network: {
        ...
        pointerDatagramGaps: Math.floor(t / 12),
        pointerDatagramJitterUs: Math.floor(700 + (Math.sin(t) + 1) * 900),
        pointerJitterP50Us: 1000,
        pointerJitterP95Us: 5000,
        pointerJitterP99Us: 10000,
        pointerDatagramMaxSilenceMs: 18,
        pointerRecoveryMilliunits: Math.floor(t * 2),
        reliableDatagramsOutbound: Math.floor(t * 2),
        reliableDatagramsInbound: Math.floor(t * 2),
        reliableDatagramRetransmits: Math.floor(t / 20),
      },
      capture: {
